...
function parser(address) {
  ...
  let streetString = "";
  const usStreetDirectionalString = Object.keys(US_STREET_DIRECTIONAL).map(x => US_STREET_DIRECTIONAL[x]).join('|');
  const usLine2String = Object.keys(US_LINE2_PREFIXES).join('|');
